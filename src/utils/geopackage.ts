...
import { getSqlModule } from "../services/rrpLocal";
import { toWgs84 } from "./proj.js";
...
function createGeometryTransformer(
  ...
  if (typeof console !== "undefined") {
    console.warn(
      `GeoPackage uses unsupported spatial reference ID ${srs}; geometries will not be reprojected.`
    );
  }
...
export async function loadGeoPackageFeatureCollection(
  ...
  try {
    ...
    for (const row of result[0].values) {
      ...
      const properties: GeoJsonProperties = props as GeoJsonProperties;
      features.push({
        type: "Feature",
        id,
        geometry: projected,

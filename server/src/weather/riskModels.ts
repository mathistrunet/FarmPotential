...
export function empiricalProbability(values: Array<number | null | undefined>, predicate: (value: number) => boolean): ProbabilityResult {
...
export function probabilityBelowQuantile(values: Array<number | null | undefined>, threshold: number): ProbabilityResult {
  return empiricalProbability(values, (value) => value <= threshold);
}

export function probabilityAboveQuantile(values: Array<number | null | undefined>, threshold: number): ProbabilityResult {
  return empiricalProbability(values, (value) => value >= threshold);
}
...
export function linearTrend(xs: number[], ys: Array<number | null | undefined>): LinearTrendResult | null {
...
export function mannKendall(values: Array<number | null | undefined>): MannKendallResult | null {
  const filtered = values.filter((value): value is number => value != null && Number.isFinite(value));
...
export function trendDirection(slope: number | null | undefined, tolerance = 1e-3): TrendDirection {
  if (slope == null || !Number.isFinite(slope)) return '→';
  if (slope > tolerance) return '↑';
  if (slope < -tolerance) return '↓';
  return '→';
}
...
export interface ConfidenceInput {
  ...
  interStationStd?: number | null; // e.g. °C or mm
}

export function computeConfidenceScore({ nYears, completeness, interStationStd }: ConfidenceInput): number {
...
export function buildBinaryRisk(label: string, values: Array<number | null | undefined>, predicate: (value: number) => boolean): RiskSummary {
  return {
    label,

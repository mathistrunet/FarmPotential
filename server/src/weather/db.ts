...
function initialiseSchema(database: DatabaseInstance) {
  database.exec(`
...
export function upsertStations(stations: Station[]): void {
  ...
  const insertMany = database.transaction((rows: Station[]) => {
...
export function upsertObservations(stationId: string, observations: ObservationRecord[]): void {
  ...
  const insertMany = database.transaction((rows: ObservationRecord[]) => {
    for (const row of rows) {
      statement.run({
...
export function listObservations(stationId: string, startISO: string, endISO: string): ObservationRecord[] {
  const database = getDatabase();
  const statement = database.prepare(
    `SELECT station_id, ts, t, tmin, tmax, rr, rr24, wind, wind_gust, humidity, pressure
     FROM observations
     WHERE station_id = ? AND ts BETWEEN ? AND ?
     ORDER BY ts ASC`
  );
  const rows = statement.all(stationId, startISO, endISO) as Array<Record<string, unknown>>;
  return rows.map((row) => ({
    stationId: row.station_id as string,
    ts: row.ts as string,
    t: row.t as number | null,
    tmin: row.tmin as number | null,
    tmax: row.tmax as number | null,
    rr: row.rr as number | null,
    rr24: row.rr24 as number | null,
    wind: row.wind as number | null,
    windGust: row.wind_gust as number | null,
    humidity: row.humidity as number | null,
    pressure: row.pressure as number | null,
  }));
}

export function getCachedRequest(hash: string, ttlHours: number): unknown | null {
  const database = getDatabase();
  const row = database
    .prepare('SELECT payload, created_at FROM cache_requests WHERE hash = ?')
    .get(hash) as { payload: string; created_at: string } | undefined;
  if (!row) return null;
...
export function setCachedRequest(hash: string, payload: unknown): void {
  const database = getDatabase();
  database
...
export function clearCacheOlderThan(ttlHours: number): void {
  const database = getDatabase();

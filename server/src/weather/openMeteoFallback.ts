...
function convertKmHToMs(value: unknown): number | null {
...
interface OpenMeteoResponse {
  hourly?: {
    ...
    windgusts_10m?: Array<number | null>;
...
export async function fetchOpenMeteoObservations(
  latitude: number,
  longitude: number,
  startISO: string,
  endISO: string
  ...
  try {
    ...
    url.searchParams.set(
      'hourly',
      'temperature_2m,relativehumidity_2m,windspeed_10m,windgusts_10m,pressure_msl,precipitation'
    );
    ...
    const dailyMinMap = new Map<string, number>();

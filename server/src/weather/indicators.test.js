...
import { aggregateIndicators, indicatorsForSlice, maxRun, computeHeatwaveStats, } from './indicators.js';
function createObservation(year, dayOfYear, values) {
...
function createYearIndicators(year, rainfallTotal) {
    return {
        year,
...
describe('aggregateIndicators', () => {
    const dataset = Array.from({ length: 10 }, (_, index) => createYearIndicators(2010 + index, 100 + index * 10));
